...
class ThreadRepository(BaseRepository[ThreadId, Thread, ThreadModel]):
    ...
    def _to_entity(self, model: ThreadModel) -> Thread:
        ...
        return Thread(
            ...
            participant_ids=[UUID(pid) for pid in model.participant_ids] if model.participant_ids else [],
    ...
    def count_filtered(
        ...
        """统计过滤后的线程数量。"""
        return self._build_filtered_query(
            statuses=statuses,
            risk_levels=risk_levels,
            tags=tags,
        ).count()
    ...
    def _build_filtered_query(
        ...
        if owner_id is not None:
            query = query.filter(ThreadModel.owner_id == owner_id)
        ...
        if tags:
            query = query.filter(or_(*[ThreadModel.tags.contains([tag]) for tag in tags]))
