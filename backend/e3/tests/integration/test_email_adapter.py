...
from ...channel_adapters.base import (
...
class TestGmailAdapter:
    ...
    def test_em_r01_fetch_message(self, gmail_adapter):
        """EM-R01: 收取新邮件并解析"""
        ...
        msg = gmail_adapter.fetch_message('test-msg-id')
    ...
    def test_em_r02_fetch_thread(self, gmail_adapter):
        """EM-R02: 收取邮件线程"""
        ...
        messages = gmail_adapter.fetch_thread_messages('test-thread-id')

        assert len(messages) == 2
        assert messages[0].thread_key == 'test-thread-id'

    def test_em_s01_send_simple_email(self, gmail_adapter):
        """EM-S01: 发送简单邮件"""
        mock_sent = {
            'id': 'sent-msg-id',
        ...
        result = gmail_adapter.send_message(
            payload={
    ...
    def test_em_s03_idempotent_send(self, gmail_adapter):
        ...
        # 第一次发送
        result1 = gmail_adapter.send_message(
        ...
        result2 = gmail_adapter.send_message(
            payload={'to': 'test@test.com', 'subject': 'Test', 'body': 'Test'},
            idempotency_key='same-key'
        )
