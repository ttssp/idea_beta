...
class TestGoogleCalendarAdapter:
    ...
    def test_ca_r01_get_free_busy(self, calendar_adapter):
        """CA-R01: 查询空闲时间"""
        ...
        result = calendar_adapter.get_free_busy(
    ...
    def test_ca_r02_list_events(self, calendar_adapter):
        """CA-R02: 列出事件"""
        mock_events = {
            'items': [
                {
        ...
        events = calendar_adapter.list_events(calendar_id='primary')

        assert len(events) == 2
        assert events[0]['summary'] == 'Test Event 1'

    def test_ca_w01_create_event(self, calendar_adapter):
        """CA-W01: 创建日历事件"""
        mock_event = {
            'id': 'new-event-id',
        ...
        result = calendar_adapter.send_message(
            payload={
                'action': 'create',
                'calendar_id': 'primary',
                'event': {
    ...
    def test_ca_w02_update_event(self, calendar_adapter):
        """CA-W02: 更新日历事件"""
        mock_event = {
            'id': 'event-id',
    ...
    def test_ca_w03_cancel_event(self, calendar_adapter):
        """CA-W03: 取消日历事件"""
        result = calendar_adapter.send_message(
            payload={
                'action': 'cancel',
    ...
    def test_ca_i01_idempotent_create(self, calendar_adapter):
        """CA-I01: 幂等创建事件"""
        existing_event = {
            'id': 'existing-id',
            'iCalUID': 'test-idempotency-key@our-system.local',
    ...
    def test_ca_i02_sequence_number(self, calendar_adapter):
    ...
    def test_fetch_event_as_message(self, calendar_adapter):
        ...
        msg = calendar_adapter.fetch_message('primary:event-id')
